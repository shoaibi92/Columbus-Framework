...
{
    /// <summary>
    /// Class SequencedHeaderDetailRepository.
    /// </summary>
        ...
        where TDetail : ModelBase, new()
    {
        ...
        protected SequencedHeaderDetailRepository(Context context, ModelHierarchyMapper<THeader> mapper, Expression<Func<THeader, bool>> activeFilter, BusinessEntitySessionParams businessEntitySessionParams)
        ...
        /// <param name="businessEntitySessionParams">BusinessEntitySessionParams</param>
        protected SequencedHeaderDetailRepository(Context context, DBLinkType dbLinkType, ModelHierarchyMapper<THeader> mapper,
        ...
        /// <returns></returns>
        public virtual EnumerableResponse<THeader> GetHeader(PageOptions<THeader> pageOptions)
        {
            CheckRights(_headerEntity, SecurityType.Inquire);
            ClearBusinessEntities(ClearLevel.Header);
            var resultsCount = SetFilter(_headerEntity, pageOptions.Filter, _headerActiveFilter, pageOptions.OrderBy);
            _headerEntity.Fetch(false);

            Search(_detailEntity, _detailActiveFilter, pageOptions.OrderBy);

            return new EnumerableResponse<THeader>
            {
                Items = Get(new List<IBusinessEntity> { _headerEntity, _detailEntity }, _mapper, pageOptions, resultsCount, resultsCount, ValidRecordFilter),
        ...
        /// <param name="pageOptions">The extra options.</param>
        /// <returns>List of details</returns>
        public virtual EnumerableResponse<TDetail> GetDetail(PageOptions<TDetail> pageOptions)
        {
            IsSessionAvailable();
        ...
        /// <returns>Header/Detail record</returns>
        public virtual EnumerableResponse<THeader> Get(int currentPageNumber, int pageSize, Expression<Func<THeader, bool>> filter = null, OrderBy orderBy = null)
        {
            CheckRights(_headerEntity, SecurityType.Inquire);
            ClearBusinessEntities(ClearLevel.Header);
        ...
        /// <returns>List of details</returns>
        public virtual EnumerableResponse<TDetail> GetDetail(int pageNumber, int pageSize, Expression<Func<TDetail, bool>> filter = null, OrderBy orderBy = null)
        {
            IsSessionAvailable();
            ...
            if (_detailEntity.Fetch(false))
            {
                // Removed a call to method GetTotalRecords because of Performance issue. Now Added below code to get Number of Details from the Header itself
                // This would return total count of detail no matter it's inserted to database or exists in memory
                // Fixed as part of the defect D-29494 : Purchase Orders Screens > Performance issues in PO Screens
                // Note : The overridden method of GetDetailCount should have actuall code of getting the count from Header, If not then just return 0 so that we still 
                //        have the existing code working
                var detailsCount = GetDetailCount(_headerEntity);
                return new EnumerableResponse<TDetail>
                {
                    Items = MapDataToModel(_detailEntity, _detailMapper, pageNumber, pageSize, resultsCount),
                    //TotalResultsCount = GetTotalRecords(_detailEntity)
                    TotalResultsCount = (detailsCount == 0) ? GetTotalRecords(_detailEntity) : detailsCount
                };
            }
