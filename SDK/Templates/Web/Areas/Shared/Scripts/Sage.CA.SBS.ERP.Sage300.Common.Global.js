...
$.extend(sg.utls, {
    ...
    logOut: function () {
        ...
        sage.cache.clearAll();
        sg.utls.ajaxPost(signOutLink, {}, function (result) {
            if (result.UserMessage.IsSuccess) {
                var topWnd = window.top == null ? window : window.top;
                topWnd.location.href = loginLink;
                //All sessions will be destroyed so no need fire page beforeunload/unload event.
                //Appears that we still need at least beforeunload event anyway.
                //In Chrome we unbind it too early and do not display "Do you want to leave this page?" dialog after changes made.
                if (!sg.utils.isChrome()) {
                    sg.utls.unbindEvent('beforeunload');
                    sg.utls.unbindEvent('unload');
                }
            }
        });
    },
