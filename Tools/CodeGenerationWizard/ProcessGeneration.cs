...
namespace Sage.CA.SBS.ERP.Sage300.CodeGenerationWizard
{
    ...
    class ProcessGeneration
    {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (_session == null)
                    {
                        ...
                        _session.InitEx(null, string.Empty, "XY", "XY1000", settings.Version);
        ...
        public static string GetDefaultName(string user, string password, string company, string version, string viewId)
        {
            ...
            try
            {
                session.InitEx(null, string.Empty, "XY", "XY1000", version);
        ...
        private bool SaveResxFile(BusinessView view, string fileName, bool addDescription)
        {
            ...
            if (writeFile)
            {
                try
                {
                    ...
                    foreach (var node in view.Fields.Select(field => new ResXDataNode(field.Name, addDescription ? field.Description : string.Empty)).Where(node => !uniqueList.Contains(node.Name)))
                    {
                    ...
                    foreach (var enumHelper in view.Enums.Values)
                    {
                        foreach (var value in enumHelper.Values)
                        {
                            ...
                            if (string.IsNullOrEmpty(key) || uniqueList.Contains(key))
                            {
                                continue;
                            }
